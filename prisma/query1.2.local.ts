...
const main = async () => {
  let x = 'Event Name,,External Team Reg,External Team Part,Internal Team Reg,Internal Team Part,,External Reg,External Part,1st Reg,1st Part,2nd Reg,2nd Part,2nd Lateral Reg,2nd Lateral Part,3rd Reg,3rd Part,3rd Lateral Reg,3rd Lateral Part,4th Reg,4th Part,4th Lateral Reg,4th Lateral Part\n'
  ...
  for (const event of events) {
    ...
    for (const team of e.Teams) {
      if (team.TeamMembers[0]?.User.collegeId !== 1) {
        table.team.external.reg++
        if (team.attended)
          table.team.external.part++
      } else {
        table.team.internal.reg++
        if (team.attended)
          table.team.internal.part++
      }
